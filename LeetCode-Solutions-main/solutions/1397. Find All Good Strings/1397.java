class Solution {
  public int findGoodStrings(int n, String s1, String s2, String evil) {
    // dp[i][j][k1][k2] := the number of good strings for s[i..n), where
    // there're j matches with the `evil`, k1 is the 0/1 tight constraint for
    // the `s1`, and k2 is the 0/1 tight constraint for the `s2`
    dp = new Integer[n][evil.length()][2][2];
    // nextMatchedCount[i][j] := the number of next matched evil count, where
    // there're j matches with `evil` and the current char is ('a' + j)
