class MKAverage {
  ...
  public void addElement(int num) {
    ...
    Move item(s) from the `mid` to the `top` to fill k slots.while (!mid.isEmpty() && topSize < k) {
      midSum -= mid.lastKey();
      add(top, remove(mid, mid.lastKey()));
      ++topSize;
    }

    // Rebalance the `mid` and the `top`.
    ...
    // Move item(s) from the `mid` to the `bot` to fill k slots.
