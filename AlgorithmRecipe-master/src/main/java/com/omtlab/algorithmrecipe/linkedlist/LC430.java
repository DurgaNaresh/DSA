 ...
 *
 * Get more details here : https://www.geeksforgeeks.org/flatten-a-linked-list-with-next-and-child-pointers/
 *
...
public class LC430 {
    ...
    public Node flatten(Node head) {
        ...
        Node finalResult = output;
        Stack<Node> stack = new Stack<>(); // Running DFS
        stack.push(head);
        ...
        while(!stack.isEmpty()) {
            ...
            output = output.next;
            if(output.next != null) {
                stack.push(output.next);
            }
            Node child = output.child;
            while (child != null) {
                stack.push(child);
