...
public class LC98 {
    ...
    public static class NodeMinMax{
        public int min = Integer.MIN_VALUE;
        public int max = Integer.MAX_VALUE;
        public boolean isValid;
        
        public NodeMinMax(){}
        public NodeMinMax(int min, int max, boolean isValid){
            this.min = min;
            this.max = max;
    ...
    public boolean isBST(TreeNode root){
        NodeMinMax minMax = inOrder(root);
        return minMax.isValid;
    }
    
    
    public NodeMinMax inOrder(TreeNode node){
        if(node == null){
            return null;
        }
        
        NodeMinMax leftN = inOrder(node.left);
        if(leftN != null && leftN.max >= node.integerValue){
            return new NodeMinMax();
        }
        
        NodeMinMax rightN = inOrder(node.right);
        if(rightN != null && rightN.min <= node.integerValue){
            return new NodeMinMax();
        }
        
        Integer min  = node.integerValue;
        Integer max = node.integerValue;
        if(leftN != null){
            min = leftN.min;
        }
        if(rightN != null){
