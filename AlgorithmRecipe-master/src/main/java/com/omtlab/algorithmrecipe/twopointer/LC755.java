 ...
 * be partial water being spread out evenly on more than 1 grid block - each unit of water has to be in exactly one 
 * block.
 *
 * Example 1:
...
public class LC755 {
    ...
    // Idea is simple. 
    // Imagine water drop moves left, 
    // and then moves right, 
    // and then moves left to position K. 
    // The position it stops is where it will stay.
    public int[] pourWater(int[] heights, int V, int K) {
        for(int i=0; i < V; i++) {
            int currentIndex = K; // Resetting currentIndex to K at every drop
            ...
            // Moving to K to level down. 
            while(currentIndex > K && heights[currentIndex-1] <= heights[currentIndex]) {
                currentIndex--;
            }
